...
public class MatchDetailActivity extends BaseActivity {
    ...
    private TextView title, teamType, entryFee, version, matchType, map, matchTime,
            ...
            joinButton, tabDescription, tabJoinedMember, noJoinedMembers;
    private ProgressBar slotsProgress, loadingSpinner;
    private View roomInfoCard, descriptionTabContent, joinedMemberTabContent;
    private LinearLayout joinedMemberList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tabDescription = findViewById(R.id.detailTabDescription);
        tabJoinedMember = findViewById(R.id.detailTabJoinedMember);
        descriptionTabContent = findViewById(R.id.descriptionTabContent);
        joinedMemberTabContent = findViewById(R.id.joinedMemberTabContent);
        joinedMemberList = findViewById(R.id.joinedMemberList);
        noJoinedMembers = findViewById(R.id.noJoinedMembers);

        tabDescription.setOnClickListener(v -> showTab(true));
        tabJoinedMember.setOnClickListener(v -> showTab(false));
    ...
    /** Switches between the Description and Joined Member tabs, matching the old app's two-tab header. */
    private void showTab(boolean showDescription) {
        descriptionTabContent.setVisibility(showDescription ? View.VISIBLE : View.GONE);
        joinedMemberTabContent.setVisibility(showDescription ? View.GONE : View.VISIBLE);
        tabDescription.setTextColor(getResources().getColor(showDescription ? R.color.white : R.color.newdarkgrey));
        tabJoinedMember.setTextColor(getResources().getColor(showDescription ? R.color.newdarkgrey : R.color.white));
    }
    ...
    private void fetchDetail() {
        ...
        String url = apiBase + "match_detail.php";
    ...
    private void render(Tournament t, JSONArray joinedUsernames) {
        ...
        boolean hasRoom = !TextUtils.isEmpty(t.getRoomDescription());
        roomId.setText(hasRoom ? t.getRoomDescription() : "-");
        roomPassword.setVisibility(View.GONE);
        roomRevealsSoon.setVisibility(t.isJoined() && !hasRoom && !t.isCompleted() ? View.VISIBLE : View.GONE);
        ...
        joinedMemberList.removeAllViews();
        int count = joinedUsernames != null ? joinedUsernames.length() : 0;
        for (int i = 0; i < count; i++) {
            String name = joinedUsernames.optString(i, "");
            if (names.length() > 0) names.append(", ");
            names.append(name);
            addJoinedMemberRow(i + 1, name);
        }
        joinedPlayers.setText(names.length() > 0 ? names.toString() : getString(R.string.no_matches_found));
        noJoinedMembers.setVisibility(count == 0 ? View.VISIBLE : View.GONE);

        bindJoinButton(t);
    }

    private void addJoinedMemberRow(int number, String userName) {
        View row = LayoutInflater.from(this).inflate(R.layout.item_joined_member_row, joinedMemberList, false);
        ((TextView) row.findViewById(R.id.joined_row_number)).setText(number + ".");
        ((TextView) row.findViewById(R.id.joined_row_name)).setText(userName);
        joinedMemberList.addView(row);
    }
    ...
    private void bindJoinButton(Tournament t) {
        ...
        } else if (t.isFull() || t.isCompleted() || t.isOngoing() || t.isCancelled()) {
    ...
    private void copyRoomPassword() {
        if (current == null || TextUtils.isEmpty(current.getRoomDescription())) return;
        android.content.ClipboardManager clipboard =
                (android.content.ClipboardManager) getSystemService(CLIPBOARD_SERVICE);
        clipboard.setPrimaryClip(android.content.ClipData.newPlainText("room_description", current.getRoomDescription()));
